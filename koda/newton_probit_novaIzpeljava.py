...
def izracunaj_koeficiente_probit(max_iteracij, matrika, vektor_rezultatov,vektor_skupin = [], zacetni_beta = [], epsilon=0.001):
    ...
    def hessian_koef(y,m,x):
        phi = norm.cdf(x)
        koef = np.divide((y-m*phi)*(-norm.pdf(x)*x*phi*(1-phi) - (norm.pdf(x) ** 2)*(1-2*phi))-m*((norm.pdf(x))**2)*phi*(1-phi),(phi*(1-phi))**2)
        return np.diag(koef)



#np.divide(y-2*y*phi-norm.pdf(x)*x*y - m*phi+2*m*phi**2 + m*phi*norm.pdf(x)*x+norm.pdf(x)*phi*(1-phi), phi*(1-phi)*m)
