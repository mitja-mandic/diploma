...
\begin{equation*}
    \log \left(\frac{p}{1-p}\right) = \mathrm{X}\beta
\end{equation*}
Vektorsko definiramu tudi
\[
    \exp{\mathrm{X}\beta} = \begin{bmatrix}
                            \exp{x_{1}^\top\beta} \\
                            \vdots\\
                            \exp{x_{n}^\top\beta}
...
\begin{align} \label{ell}
    ...
    &= y^\top\mathrm{X}\beta - n^\top \log(1 + \exp{\mathrm{X}\beta}),
\end{align}
in še odvoda zgornje funkcjie, ki pa ga lahko zapišemo kot
\begin{align}
    \dot{\ell}(\beta) = \mathrm{X}^\top(y - m\circ p(\beta)),
\end{align}
...
\begin{align}\label{prvi}
    \dot{\ell}(\beta) = X^\top(y - m \circ p(\beta)) = X^\top(y - \mu(\beta))
\end{align}
...
\begin{equation} \label{drugi}
    \ddot{\ell}(\beta) = -\mathrm{X}^\top v(\beta)\mathrm{X},
\end{equation}
...
\[
    I(\theta) = \mathrm{E}[\dot{\ell}(\theta) \dot{\ell}(\theta)^\top ]. %= \mathrm{E}[\dot{\ell}(\theta) \dot{\ell}(\theta)^\top].
\]

Fisher scoring algoritem sedaj je po zgornjih oznakah 
\begin{align}
    \theta{n + 1} = \theta_{n} - I(\theta)^{-1}\dot{\ell}(\theta)
\end{align}
...
\[
    I(\theta) = \mathrm{E}[-\ddot{\ell}(\theta)] \overset{\eqref{drugi}}{=} X^\top v(\theta)X = -\ddot{\ell}(\theta),
\]
...
\begin{align}
    \hat{\beta}_{i+1} = \hat{\beta}_{i} - (\ddot(\ell))
\end{align}
