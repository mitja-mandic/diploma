...
ter predstavimo metode za ocenjevanje parametrov, s povdarkom na metodi največjega verjetja. Izpeljemo enačbe verjetja za logistični model, rezultate
nato komentiramo v luči eksponentne družine in jih posplošimo za vse porazdelitve, ki ji pripadajo.
Izpeljemo tudi enačbe verjetja v probit modelu in vidimo prednosti uporabe kanoničnih povezovalnih funkcij. 
V drugem delu naloge se posvetimo numeričnim metodam. Izpeljemo Newtonovo metodo in komentiramo težave, ki lahko nastopijo z njeno uporabo. Definiramo tudi Fisher-scoring algoritem
in dokažemo, da se ob uporabi modela s kanonično povezovalno funkcijo ujema z Newtonovo metodo.
...
\section{Uvod}
Kaj natanko se zgodi v računalniškem ozadju, ko v programskem paketu poženemo ukaz za izračun koeficientov največjega verjetja? Kateri algoritmi se uporabljajo in 
...
za računanje koeficientov cenilk, podrobneje pa izpeljemo enačbe verjetja v logističnem in probit modelu. 

V sledečih poglavjih izpeljemo numerične metode - Newtonovo metodo po korakih izpeljemo in komentiramo njene prednosti in slabosti ter jo nadgradimo v Fisher-scoring algoritem,
ki ga tudi implementiramo in rezultate predstavimo v zadnjem poglavju.
...
$X^{q}$ matrika teh opažanj dimenzij $r\times(p+1).$ Velja $X^{q}\beta=0,$ torej so vrstice v $X^{q}$ linearno odvisne in pripadajo podprostoru dimenzije $d \leq p-1$.
Velja tudi $d = rang(X^{q}) - 1.$ Če je $rang(X^{q}) = p$ je torej ločnica med podatki enolična, kot v sliki \ref{fig:enolicna}, sicer pa je takih ločnic lahko več, kot vidimo v
...
\begin{align}
    ...
    &= \mathrm{E}[\left(\frac{\partial}{\partial\partial\theta} L(\theta) - \mathrm{E}[\frac{\partial}{\partial\theta} L(\theta)]\right)\left(\frac{\partial}{\partial\theta} L(\theta) - \mathrm{E}[\frac{\partial}{\partial\theta} L(\theta)]\right)^\top] \nonumber \\
