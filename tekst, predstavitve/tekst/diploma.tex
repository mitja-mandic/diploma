...
\section{Eksponentna družina} %mal razložim, predvsem da bom kasneje dokazal enakost N-R in F-S
...
\begin{equation}
    f_{Y}(y; \theta, \phi) = \exp{\left(\frac{A(y\cdot\theta - \gamma(\theta))}{\phi} + \tau(y, \frac{\phi}{A})\right)},
\end{equation}
za vektorski parameter $\theta,$ ki ga pogosto imenujemo \textit{kanonični} ali \textit{naravni} parameter, disperzijski parameter $\phi$, utež $A,$
ter dani funkciji $\gamma, \tau.$ 

Koristno je tudi pogledati logaritem zgornje enačbe
\begin{align}
    \ell(\theta) &= \log f_{Y}(y; \theta, \phi) = A\frac{y\cdot\theta - \gamma{\theta}}{\phi} + \tau(y,\frac{\phi}{A}) \\
    \frac{\partial}{\partial \theta}\ell &= \frac{A}{\phi}(y - \gamma'(\theta))
\end{align}
Izkaže se, da za slučajne vektorje z gostoto iz eksponentne družine veljata naslednji zvezi %še nevem če bom dokazoval. VPRAŠAJ SMREKIJA
\begin{equation}\label{izp}    
    \mathbb{E}(Y) = \mu = \gamma'(\theta), \mathrm{Var}(Y) = \frac{\phi}{A}\gamma''(\theta)
\end{equation}

Poleg formul za izračun variance in pričakovane vrednosti nam zgornji rezultat pove še, da je za porazdelitve eksponentne družine varianca funkcija pričakovane vrednosti.
...
\begin{itemize}
    ...
    \begin{align*}
        \ell(\mu, \sigma; y) &= \log\left(\frac{1}{\sqrt{2\pi\sigma^2}}\right) -\frac{(y-\mu)^2}{2\sigma^2} = -\frac{1}{2}\log(2\sigma^2\pi) - \frac{y^2 - 2\mu y + \mu^2}{2 \sigma^2}    \\ \nonumber
                            &= \frac{y\mu-\nicefrac{\mu^2}{2}}{\sigma^2} - \frac{1}{2}\left(\frac{y^2}{\sigma^2} + \log(2\pi\sigma^2)\right) 
    \end{align*}
    Od tu preberemo zgoraj definirane vrednosti
    \[
        \theta = \mu,~\phi = \sigma^2,~\gamma(\theta) = \frac{\theta^2}{2},~A = 1,
    \]
    in takoj sledijo zaključki
    \[
        \mathbb{E}(Y) = \gamma'(\theta) = \mu \text{ in } \mathrm{Var}(Y) = \frac{\phi}{A}\gamma''(\theta) = \sigma^2,
    \]
    kar se ujema z enačbami \refeq{izp}.
    ...
    \[
        \theta = \log\frac{p}{1-p},~\gamma(\theta) = n\log(1 + e^{\theta}),~\phi = 1,
    \]
