...
predvideti trende v prihodnosti. Modeli morajo biti kar se da enostavni, ampak hkrati zagotavljati določeno natančnost. Ogledali si bomo nekaj različnih linearnih
modelov, natančneje pa spoznali \textit{logistični model} za računanje verjetnosti dogodkov. Dokazali bomo tudi nekaj zvez, ki se tičejo eksponentne družine
in kanoničih parametrov. Skozi slednje bomo utemeljili uporabo prilagojene Newtonove metode za reševanje sistema enačb verjetja, \textit{Fisher-scoring} algoritma,
kje ležijo njegove prednosti pred običajno Newtonovo metodo in kako njegova uporaba izgleda v praksi.
...
\subsubsection{Kvazi-popolna separacija}
Kvazi-popolna separacija v podatkih je prisotna, če obstaja tak vektor $\beta$, da za vse $i\in E_{j}$ in $j,t = 1,\ldots,g,~j\neq t$ velja
\[
...
%\[
%    \begin{bmatrix}
%        \frac{\phi(x_{1}^\top\beta)}{\Phi(x_{1}^\top\beta)(1-\Phi(x_{1}^\top\beta))}\left(\phi(x_{1}^\top\beta)\frac{2y_{1} - m_{1}\Phi(x_{1}^\top\beta)^2 - y_{1}}{(\Phi(1-\Phi(x_{1}^\top\beta)))}
%    -(x_{1}^\top\beta)(y_{1}-m_{1}\Phi(x_{1}^\top\beta)) \right) & & &\\
%    & \ddots & & \\
%    & & & \frac{\phi(x_{n}^\top\beta)}{\Phi(x_{n}^\top\beta)(1-\Phi(x_{n}^\top\beta))}\left(\phi(x_{n}^\top\beta)\frac{2y_{n} - m_{n}\Phi(x_{n}^\top\beta)^2 - y_{n}}{(\Phi(1-\Phi(x_{n}^\top\beta)))}
%    -(x_{n}^\top\beta)(y_{n}-m_{n}\Phi(x_{n}^\top\beta)) \right)
%    \end{bmatrix}
%\]
...
Najprej postavimo algoritem. Ta se idejno sicer ne bo bistveno razlikoval od tistega uporabljenega v prejšnjem odseku.
\begin{algorithm}[H]
...
Zanimivejše kot le naštevanje parametrov, pa je primerjanje dobljenih številk. 
\begin{center}
    \begin{figure}[H]
    \begin{tikzpicture}
        \begin{axis}[
