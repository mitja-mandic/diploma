...
\begin{align}
    \ell(\theta) &= \log f_{Y}(y; \theta, \phi) = \frac{y\theta - b(\theta)}{a(\phi)} + c(y,\phi) \\
    \frac{\partial}{\partial \theta}\ell &= \frac{(y - b'(\theta))}{a(\phi)} \\
...
\begin{equation}\label{izp}    
    \mathbb{E}(Y) = \mu = b'(\theta), \mathrm{Var}(Y) = \frac{\phi}{A}\gamma''(\theta)
\end{equation}
